...
import {
  ArrowRight, X, CheckCircle, Plus, BookOpen, Clock, Users,
...
const EMOJIS = ['BookOpen', 'GraduationCap', 'Microscope', 'Stethoscope', 'Activity', 'Brain']
const FACULTIES = ['Pharmacy', 'Business', 'Physical Therapy', 'Dentistry', 'All Faculties']
const LEVELS = ['Beginner', 'Intermediate', 'Advanced']

/* ───────── Add Course Modal — 3 Steps ──────────────────────────── */
type Step = 'info' | 'media' | 'modules'

interface Module { title: string; description: string; duration: string }
...
function StepIndicator({ step }: { step: Step }) {
  const steps: { key: Step; label: string; icon: React.ReactNode }[] = [
    { key: 'info', label: 'Basic Info', icon: <BookOpen className="w-4 h-4" /> },
      ...
      {steps.map((s, i) => (
        <div key={s.key} className="flex items-center flex-1">
          <div className={`flex items-center gap-2 px-3 py-2 rounded-xl text-xs font-semibold transition-all ${
            i <= idx ? 'bg-foreground text-background' : 'bg-secondary text-muted-foreground'
          }`}>
            {s.icon} {s.label}
          </div>
          {i < steps.length - 1 && (
            <div className={`flex-1 h-px mx-1 ${i < idx ? 'bg-foreground' : 'bg-border'}`} />
          )}
        </div>
      ))}
    </div>
...
function AddCourseModal({ onClose, onAdded }: AddCourseModalProps) {
  ...
  const [form, setForm] = useState({
    title: '', description: '', faculty: '', level: 'Beginner',
    duration: '', instructor: '', image_url: '📚', featured: false,
  })
  ...
  const addModule = () => setModules(m => [...m, { title: '', description: '', duration: '' }])
      ...
      <motion.div initial={{ opacity: 0, y: 30, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }}
        ...
        {done ? (
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
            ...
            <h3 className="font-playfair text-3xl text-foreground mb-2">Course Published!</h3>
            <p className="text-muted-foreground text-sm mb-6">Your course is now live for students to discover and enroll.</p>
            <button onClick={onClose}
          ...
          <>
            <div className="mb-1">
              ...
              <h2 className="font-playfair text-3xl text-foreground mb-6">Add a Course</h2>
            ...
            <AnimatePresence mode="wait">
                ...
                <motion.div key="info"
                  ...
                  <div>
                    ...
                    <div className="flex gap-2 flex-wrap">
                      {EMOJIS.map(e => (
                        <button type="button" key={e} onClick={() => setForm(f => ({ ...f, image_url: e }))}
                  ...
                  <div>
                    ...
                    <textarea required value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))}
                      ...
                      rows={3} className="w-full bg-secondary/40 border border-border rounded-xl px-4 py-3 text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-foreground/15 transition resize-none" />
                  </div>

                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-1.5">Faculty *</label>
                      <select required value={form.faculty} onChange={e => setForm(f => ({ ...f, faculty: e.target.value }))}
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      <label className="block text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-1.5">Instructor</label>
                  ...
                  <label className="flex items-center gap-3 cursor-pointer">
                    ...
                    <span className="text-sm text-muted-foreground">Mark as Featured Course</span>
              ...
              {step === 'media' && (
                <motion.div key="media"
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-1.5">
                      <span className="flex items-center gap-2"><Video className="w-3.5 h-3.5" /> Intro Video URL</span>
                    </label>
                    <input value={videoUrl} onChange={e => { setVideoUrl(e.target.value); setPreviewType('video') }}
                    ...
                    {videoUrl && (
                      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
                        ...
                        <div>
                          <p className="text-sm font-semibold text-foreground">Video linked</p>
                  ...
                  <div className="bg-secondary/30 rounded-xl p-4 border border-border/50">
                    <p className="text-xs text-muted-foreground leading-relaxed">
                      💡 <strong>Tip:</strong> Use Google Drive, Imgur, or any public image URL for the cover.
                      For videos, YouTube or Vimeo links work best.
                    </p>
                  </div>
                </motion.div>
              )}
              ...
              {step === 'modules' && (
                <motion.div key="modules"
                  ...
                  <div className="flex items-center justify-between mb-2">
                    <p className="text-sm text-muted-foreground">Add your course curriculum modules</p>
                        ...
                        <motion.div key={i}
                          ...
                          <div className="flex items-start justify-between gap-2 mb-3">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Module {i + 1}</span>
                            </div>
                            {modules.length > 1 && (
                          ...
                          <div className="space-y-2">
                            ...
                            <input value={mod.description} onChange={e => updateModule(i, 'description', e.target.value)}
                              placeholder="Brief description of what's covered"
                              className="w-full bg-background/50 border border-border rounded-lg px-3 py-2 text-sm text-foreground focus:outline-none focus:ring-1 focus:ring-foreground/20" />
                            <input value={mod.duration} onChange={e => updateModule(i, 'duration', e.target.value)}
                  ...
                  <div className="bg-secondary/30 rounded-xl p-4 border border-border/50 flex items-center gap-3">
                    ...
                    <p className="text-xs text-muted-foreground">
                      You have <strong className="text-foreground">{modules.filter(m => m.title).length}</strong> module(s) added.
                      Students will see this as your course curriculum.
                    </p>
                  </div>
                </motion.div>
              )}
            ...
            <div className="flex gap-3 mt-8 pt-5 border-t border-border">
              ...
              {step !== 'modules' ? (
                <button type="button" onClick={nextStep} disabled={step === 'info' && !isStep1Valid}
                ...
                <button type="button" onClick={handleSubmit} disabled={loading}
                  ...
                  {loading ? <><Loader2 className="w-4 h-4 animate-spin" /> Publishing...</> : <>Publish Course <ArrowRight className="w-4 h-4" /></>}
                </button>
              )}
            </div>
          </>
...
/* ───────── Enroll Modal ─────────────────────────────────────────── */
interface EnrollModalProps { course: any; onClose: () => void }
      ...
      <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }}
        ...
        {done ? (
          <div className="text-center py-6">
            ...
            <h3 className="font-playfair text-2xl text-foreground mb-1">Enrolled!</h3>
            <p className="text-muted-foreground text-sm">{course.title}</p>
            <button onClick={onClose} className="mt-5 bg-foreground text-background px-8 py-2.5 rounded-xl font-semibold text-sm cursor-pointer hover:opacity-90 transition-opacity">
              Start Learning →
            </button>
          </div>
        ) : (
          <>
            ...
            <div className="flex items-center gap-3 mb-5">
              ...
              <div>
                <h2 className="font-playfair text-xl text-foreground leading-snug">{course.title}</h2>
                <div className="flex gap-2 mt-1 mb-1">
                  {course.level && <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider ${LEVEL_BADGE[course.level] || 'bg-secondary text-muted-foreground'}`}>{course.level}</span>}
                  {course.faculty && <span className="text-xs text-muted-foreground">{course.faculty}</span>}
                </div>
                {course.instructor && (
                  <div className="text-xs">
                    {course.user_id ? (
                      <a href={`/researcher/${course.user_id}`} className="font-semibold text-accent-blue hover:underline">By {course.instructor}</a>
                    ) : (
                      <span className="font-semibold text-muted-foreground">By {course.instructor}</span>
                    )}
                  </div>
                )}
              </div>
            ...
            {course.modules?.length > 0 && (
              <div className="flex items-center gap-2 text-xs text-muted-foreground mb-5 bg-secondary/50 rounded-xl px-3 py-2">
                <List className="w-4 h-4" /> {course.modules.length} modules · {course.duration || 'Self-paced'}
              </div>
            )}
            ...
            <form onSubmit={handleEnroll} className="space-y-3">
              ...
              <input required type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="University Email"
                className="w-full bg-secondary/50 border border-border rounded-xl px-4 py-3 text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-foreground/20" />
              <button type="submit" disabled={loading}
                ...
                {loading ? <><Loader2 className="w-4 h-4 animate-spin" /> Enrolling...</> : 'Enroll — Free →'}
              </button>
            </form>
...
export function Courses() {
  ...
  const filters = ['All', 'Beginner', 'Intermediate', 'Advanced', 'Business', 'Physical Therapy', 'Dentistry', 'Pharmacy']
  ...
  const filtered = courses.filter((c: any) =>
    filter === 'All' ? true :
    ['Beginner', 'Intermediate', 'Advanced'].includes(filter) ? c.level === filter :
  ...
  return (
    <section id="courses-section" className="harvard-section bg-background">
      <div className="max-w-7xl mx-auto px-6 lg:px-12">

        {/* Harvard-style intro */}
        <div className="harvard-section-intro">
          <div>
            <p className="text-xs font-semibold text-muted-foreground uppercase tracking-widest mb-3">Online Learning</p>
          ...
          <div className="flex flex-col justify-end gap-6 h-full">
            <p className="text-muted-foreground leading-relaxed text-lg italic max-w-xl">
              Faculty-curated courses from Deraya's four colleges — explore, enroll, and earn academic recognition at your own pace.
            </p>
            {/* Filters */}
            <div className="pt-2">
              <FilterDropdown options={filters} activeOption={filter} onSelect={setFilter} label="Filter Courses" colorClass="accent-red" />
            </div>
          </div>
        </div>
        ...
        {loading && (
          <div className="flex items-center justify-center py-24">
            ...
            <span className="ml-3 text-muted-foreground text-sm">Loading courses...</span>
          </div>
        )}
        ...
        {!loading && featured && (
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }} className="harvard-featured-grid mb-14">
            {/* Left: featured */}
            <div className="pr-0 lg:pr-10 border-r-0 lg:border-r border-border">
              <motion.div whileHover={{ y: -3 }} transition={{ duration: 0.2 }}
                ...
                {featured.cover_url ? (
                  ...
                  <div className={`${featured.cover_url ? 'absolute bottom-0 left-0 right-0 p-8' : 'p-8'}`}>
                    <div className="flex flex-wrap gap-2 mb-3">
                      <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${LEVEL_BADGE[featured.level] || 'bg-secondary text-foreground'} ${featured.cover_url ? 'bg-white/20 text-white' : ''}`}>
                        {featured.level || 'Featured'}
                      </span>
                      <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${featured.cover_url ? 'bg-white/10 text-white/80' : 'bg-secondary/80 text-foreground'}`}>
                        {featured.faculty}
                      </span>
                    </div>
                    <h3 className={`font-playfair text-2xl lg:text-3xl mb-2 leading-tight ${featured.cover_url ? 'text-white' : 'text-foreground'}`}>
                      {featured.title}
                    </h3>
                  {!featured.cover_url && <p className="text-muted-foreground text-sm line-clamp-2 mb-4">{featured.description}</p>}
                  <div className={`flex flex-wrap gap-4 text-xs mb-5 ${featured.cover_url ? 'text-white/70' : 'text-muted-foreground'}`}>
                    {featured.duration && <span className="flex items-center gap-1.5"><Clock className="w-3.5 h-3.5" />{featured.duration}</span>}
                    {featured.modules?.length > 0 && <span className="flex items-center gap-1.5"><BookOpen className="w-3.5 h-3.5" />{featured.modules.length} Modules</span>}
                    <span className="flex items-center gap-1.5"><Users className="w-3.5 h-3.5" />2.4k Enrolled</span>
                  </div>
                  <button className={`inline-flex items-center gap-2 text-sm font-semibold transition-all ${featured.cover_url ? 'text-white' : 'text-foreground'}`}>
                    <span className={`w-9 h-9 rounded-full border-[1.5px] flex items-center justify-center group-hover:scale-110 transition-transform ${featured.cover_url ? 'border-white' : 'border-foreground'}`}>
            ...
            <div className="pl-0 lg:pl-10 mt-8 lg:mt-0">
              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-widest mb-4">More Courses</p>
              {rest.slice(0, 6).map((course: any) => (
                <motion.div key={course.id} whileHover={{ x: 3 }}
                  className="harvard-list-item group" onClick={() => setEnrollTarget(course)}>
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <h4 className="font-medium text-foreground text-sm leading-snug group-hover:underline">{course.title}</h4>
                      <p className="text-xs text-muted-foreground mt-0.5">{course.faculty} · {course.level}</p>
                    </div>
                  </div>
              ...
              {rest.length === 0 && (
                <div className="text-center py-10 text-muted-foreground text-sm">
                  ...
                  <button onClick={() => setShowAdd(true)} className="underline cursor-pointer">Add one!</button>
                </div>
              )}
            </div>
          </motion.div>
        ...
        {!loading && courses.length === 0 && (
          ...
          >
            ...
            <h3 className="font-playfair text-2xl text-foreground mb-3">No courses yet</h3>
            <p className="text-muted-foreground mb-6 text-sm">Be the first to share academic knowledge with the Deraya community.</p>
            <motion.button onClick={() => setShowAdd(true)} whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.96 }}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filtered.map((course: any, i: number) => (
              <motion.div key={course.id}
                ...
                whileHover={{ y: -10, rotateX: 2 }} className="harvard-card pt-5 group cursor-pointer card-3d"
                ...
                <div className="flex items-start justify-between mb-3">
                  ...
                  <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${LEVEL_BADGE[course.level] || 'bg-secondary text-muted-foreground'}`}>
                    {course.level}
                  </span>
                </div>
                <h3 className="font-playfair text-xl text-foreground mb-2 leading-snug group-hover:underline">{course.title}</h3>
                <p className="text-muted-foreground text-sm line-clamp-2 mb-3">{course.description}</p>
                <p className="text-xs text-muted-foreground mb-4">{course.faculty}</p>
                <button className="harvard-arrow-btn" onClick={e => { e.stopPropagation(); setEnrollTarget(course) }}>
                  <span className="harvard-arrow-circle"><ArrowRight className="w-3.5 h-3.5" /></span>
                  Enroll — Free
                </button>
              </motion.div>
            ))}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {rest.slice(6).map((course: any, i: number) => (
                <motion.div key={course.id}
                  initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }}
                  viewport={{ once: true }} transition={{ delay: i * 0.05 }}
                  whileHover={{ y: -4 }} className="harvard-card pt-5 group cursor-pointer"
                  onClick={() => setEnrollTarget(course)}>
                  <div className="flex items-start justify-between mb-3">
      ...
      <AnimatePresence>
        {showAdd && <AddCourseModal onClose={() => setShowAdd(false)} onAdded={c => { setCourses(prev => [c, ...prev]); setShowAdd(false) }} />}
        {enrollTarget && <EnrollModal course={enrollTarget} onClose={() => setEnrollTarget(null)} />}
      </AnimatePresence>
    </section>
  )
}
