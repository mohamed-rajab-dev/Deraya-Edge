...
const SECTIONS = [
  { label: 'Research Papers', href: '/research', desc: 'Peer-reviewed & non-peer academic publications' },
  { label: 'Courses', href: '/courses', desc: 'Faculty-curated academic learning' },
  ...
  { label: 'Internship', href: '/internship', desc: 'Opportunities & placements' },
]
...
export function Hero() {
  ...
  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault()
    if (searchVal.trim()) navigate(`/research?q=${encodeURIComponent(searchVal.trim())}`)
  }
  ...
  return (
    <div ref={heroRef} className="relative min-h-screen w-full bg-background overflow-hidden">
      ...
      <motion.div style={{ y, opacity }} className="relative z-10 max-w-7xl mx-auto px-6 lg:px-12 pt-44 pb-24">
        {/* Tag */}
        ...
        >
          ...
          <div className="flex flex-col items-start" style={{ perspective: '1200px' }}>
            <motion.h1 
        ...
        <motion.div
          ...
          {SECTIONS.map((s, i) => (
            ...
            >
              <div className="font-playfair font-black text-foreground text-xs mb-1 group-hover:text-accent-red transition-colors tracking-widest uppercase">{s.label}</div>
              <div className="text-muted-foreground text-[10px] leading-tight font-playfair italic opacity-60 group-hover:opacity-100 transition-opacity">{s.desc}</div>
            </motion.a>
          ))}
        </motion.div>
