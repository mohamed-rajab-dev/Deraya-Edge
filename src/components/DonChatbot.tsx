...
export function DonChatbot() {
  ...
  const [messages, setMessages] = useState<Msg[]>([
    { role: 'assistant', content: "Hi! I'm **Don**, your research assistant.\n\nHow can I help you today?" }
  ])
  ...
  const send = async () => {
    ...
    try {
      ...
      if (!resp.ok) {
        setMessages(prev => [...prev, { role: 'assistant', content: `[Error] Failed to get response from Don.` }])
        setIsLoading(false)
        return
      }
      ...
      setMessages(prev => [...prev, { role: 'assistant', content: '[Error] Connection error. Please make sure the backend is running.' }])
    }
    setIsLoading(false)
  }
    ...
    <>
      ...
      <button
        ...
        className={`fixed bottom-6 left-6 z-[150] w-14 h-14 rounded-full flex items-center justify-center elevated-shadow transition-all duration-300 cursor-pointer ${
          isOpen ? 'bg-foreground text-background rotate-0' : 'bg-destructive text-destructive-foreground hover:scale-110'
        }`}
      >
