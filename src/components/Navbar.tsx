...
import { Menu, X, User, Globe, LogOut, Sun, Moon, Search, ChevronDown } from 'lucide-react'
...
export function Navbar({ onSearch }: NavbarProps) {
  ...
  return (
    <>
      ...
      >
        <div className={`mx-auto max-w-[880px] transition-all duration-500 rounded-[22px] border ${isScrolled ? 'glass-navbar shadow-2xl py-0.5 px-3 border-white/5' : 'bg-transparent py-1.5 px-2 border-transparent'}`}>
          <div className="max-w-7xl mx-auto px-4 lg:px-5 py-2 flex items-center justify-between gap-4">

            {/* ── Creative Logo (Smaller) ── */}
            <Link to="/" className="flex items-center gap-0.5 flex-shrink-0 group relative overflow-hidden">
              <span className="font-playfair text-2xl font-black text-foreground tracking-tighter">Deraya</span>
              <div className="ml-0.5 px-2 py-0.5 bg-accent-blue rounded-md shadow-lg shadow-accent-blue/20 flex items-center justify-center group-hover:scale-105 transition-transform">
                <span className="font-playfair text-[14px] font-black text-white italic tracking-tight">Edge</span>
              </div>
            </Link>
            ...
            <div className="hidden lg:flex items-center flex-1 justify-center">
              <div className="flex items-center gap-0.5">
                ...
                {/* More Dropdown */}
                <div className="relative ml-1" onMouseEnter={() => setIsMoreOpen(true)} onMouseLeave={() => setIsMoreOpen(false)}>
            ...
            <div className="flex items-center gap-3 flex-shrink-0">
              ...
              >
                ...
                >
                  {isDark ? <Sun className="w-4 h-4 text-amber-400" /> : <Moon className="w-4 h-4 text-accent-blue" />}
                </motion.div>
              </button>
        ...
        {isMobileOpen && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <div className="flex items-center justify-between mb-10">
              <Link to="/" className="flex items-center gap-0.5">
                ...
                <div className="ml-1 px-2 py-0.5 bg-accent-blue rounded-md shadow-lg shadow-accent-blue/30">
                  <span className="font-playfair text-lg font-black text-white italic tracking-tight">Edge</span>
                </div>
              </Link>
            ...
            <div className="space-y-1 flex-1">
              {[...PRIMARY_LINKS, ...SECONDARY_LINKS].map((link, i) => (
                <motion.div key={link.label}
                  ...
                  <Link to={link.href} onClick={() => setIsMobileOpen(false)}
            ...
            <div className="pt-8 border-t border-border flex gap-3 mt-6">
              {user ? (
                <button onClick={() => { signOut(); setIsMobileOpen(false) }}
                ...
                <button onClick={() => { setIsMobileOpen(false); setShowAuth(true) }}
