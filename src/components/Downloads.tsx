		...
		<section
			...
			<div className='max-w-7xl mx-auto px-6 lg:px-12'>
				...
				<div className='flex flex-col lg:flex-row lg:items-end justify-between gap-8 mb-10'>
					<div className='max-w-xl'>
						<div className='flex items-center gap-3 mb-3'>
							...
							<span className='text-accent-blue text-sm font-bold uppercase tracking-widest'>
								Publications
							</span>
						</div>
						<h2 className='font-bagel text-4xl sm:text-5xl text-foreground mb-4 heading-shimmer'>
							Research Papers
						</h2>
						<p className='text-muted-foreground text-lg'>
							Browse and download research papers from all faculties.
						</p>
					</div>
					<button
				...
				<motion.div
					...
					<div
						className={`flex items-center gap-3 bg-card clean-border rounded-2xl px-5 py-3.5 transition-all ${isSearchFocused ? 'ring-2 ring-accent-blue/30 border-accent-blue/30' : 'hover:border-accent-blue/20'}`}>
						<Search
							className={`w-5 h-5 flex-shrink-0 transition-colors ${isSearchFocused ? 'text-accent-blue' : 'text-muted-foreground'}`}
						/>
						<input
							type='text'
							value={searchQuery}
						...
						{searchQuery && (
							<button
								...
								className='p-1 rounded-full hover:bg-secondary transition-colors cursor-pointer flex-shrink-0'>
				...
				{showForm && user && (
					<motion.div
						...
						className='bg-card clean-border rounded-2xl p-6 mb-8'>
						...
						<div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
							<input
								...
								className='bg-input clean-border rounded-xl px-4 py-3 text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-accent-blue/30'
							/>
							<select
								value={faculty}
								onChange={(e) => setFaculty(e.target.value)}
								className='bg-input clean-border rounded-xl px-4 py-3 text-foreground text-sm focus:outline-none'>
								{faculties.slice(1).map((f) => (
									<option
										key={f}
										value={f}>
										{f}
							...
							<textarea
								...
								className='bg-input clean-border rounded-xl px-4 py-3 text-foreground text-sm md:col-span-2 focus:outline-none resize-none'
								rows={3}
							/>
							<div className='flex gap-3 items-center'>
								...
								<input
									...
									placeholder='Pages'
									className='bg-input clean-border rounded-xl px-4 py-3 text-foreground text-sm w-24 focus:outline-none'
								/>
							</div>
							<div className='flex gap-2 md:col-span-2'>
								<button
									...
									disabled={submitting}
									className='bg-accent-blue text-white px-6 py-2.5 rounded-xl font-semibold cursor-pointer text-sm disabled:opacity-50 hover:bg-accent-blue/90 transition-colors'>
									{submitting ? 'Publishing…' : 'Publish'}
								</button>
								<button
									onClick={() => setShowForm(false)}
									className='bg-secondary px-6 py-2.5 rounded-xl font-semibold cursor-pointer text-sm text-foreground'>
									Cancel
								</button>
							</div>
						</div>
								...
								<motion.div
									...
									<div className='p-8'>
										...
										<p className='text-muted-foreground text-sm leading-relaxed mb-6 line-clamp-3 opacity-80'>
											{pub.abstract ||
												'Academic research publication from Deraya University.'}
										</p>
										...
										<div className='flex flex-wrap items-center gap-4 text-[10px] font-black uppercase tracking-widest text-muted-foreground pt-6 border-t border-border/50'>
											<div className='flex items-center gap-2'>
												...
												<span className='text-foreground'>
													{pub.User?.display_name || 'Researcher'}
												</span>
											</div>
				...
				{selectedPaper && (
					<PremiumReader
						...
						item={{
							...
							readTime: selectedPaper.pages
								? `${selectedPaper.pages} pages document`
								: 'Open access',
							type: 'research',
