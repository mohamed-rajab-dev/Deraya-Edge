...
export function Profile() {
	...
	const [activeTab, setActiveTab] = useState<
		'research' | 'articles' | 'posts' | 'projects' | 'courses'
	>('research');
							...
							{editingProfile ? (
								<div className='space-y-3'>
									<input
										...
										className='w-full bg-input clean-border rounded-xl px-4 py-2 text-foreground'
									/>
									<textarea
										value={editBio}
									...
									<select
										...
										className='bg-input clean-border rounded-xl px-4 py-2 text-foreground'>
									...
									<div className='flex gap-2'>
										<button
											...
											className='bg-destructive text-destructive-foreground px-4 py-2 rounded-lg text-sm font-semibold cursor-pointer'>
											Save
										</button>
										<button
											onClick={() => setEditingProfile(false)}
											className='bg-card clean-border px-4 py-2 rounded-lg text-sm font-semibold cursor-pointer'>
											Cancel
										</button>
									</div>
								</div>
								...
								<>
									...
									<button
										...
										className='text-sm text-destructive font-semibold mt-3 cursor-pointer'>
										Edit Profile
									</button>
								</>
							)}
						...
						<button
							...
							className={`flex items-center gap-2 px-5 py-2.5 rounded-full text-sm font-semibold transition-all cursor-pointer ${
								activeTab === tab.key
									...
									: 'bg-card clean-border text-muted-foreground hover:text-foreground'
							}`}>
					...
					<div className='bg-card clean-border rounded-2xl p-6 mb-6'>
						...
						<div className='space-y-4'>
							<input
								...
								placeholder={
									activeTab === 'posts' ? 'Write your post...' : 'Title'
								}
							...
							{activeTab !== 'posts' && (
								<textarea
									...
									placeholder={
										activeTab === 'research' ? 'Abstract' : 'Description'
									}
									className='w-full bg-input clean-border rounded-xl px-4 py-3 text-foreground'
									rows={3}
								/>
							...
							<div className='flex gap-3 flex-wrap'>
								...
								{activeTab === 'research' && (
									<input
										...
										placeholder='Pages'
										className='bg-input clean-border rounded-xl px-4 py-2 text-foreground w-24'
									/>
								)}
								{activeTab === 'projects' && (
									<select
							...
							<div className='flex gap-2'>
								<button
									...
									disabled={loading}
									className='bg-destructive text-destructive-foreground px-6 py-2 rounded-lg font-semibold cursor-pointer disabled:opacity-50'>
									{loading ? 'Publishing...' : 'Publish'}
								</button>
								<button
									onClick={() => setShowForm(false)}
									className='bg-card clean-border px-6 py-2 rounded-lg font-semibold cursor-pointer'>
									Cancel
								</button>
							</div>
						</div>
				...
				<div className='space-y-4'>
					{activeTab === 'research' &&
						myResearch.map((item) => (
							<div
								key={item.id}
								className='bg-card clean-border rounded-2xl p-6 flex items-start justify-between'>
								<div>
									...
									{item.file_url && (
										<a
											...
											target='_blank'
											rel='noopener noreferrer'
											className='text-sm text-accent-blue font-semibold mt-2 inline-block'>
											📄 Download PDF
										</a>
									)}
								...
								<button
									onClick={() => handleDelete('research_papers', item.id)}
									className='text-destructive text-sm font-semibold cursor-pointer flex-shrink-0 ml-4'>
									Delete
								</button>
							</div>
						))}
						...
						myArticles.map((item) => (
							<div
								...
								<div>
									...
									<p className='text-foreground mt-1 text-sm line-clamp-2'>
										{item.content}
									</p>
									<span className='text-xs text-muted-foreground mt-2 block'>
										{new Date(item.created_at).toLocaleDateString()}
					...
					{activeTab === 'posts' &&
						myPosts.map((item) => (
							<div
								...
								<button
									onClick={() => handleDelete('community_posts', item.id)}
									className='text-destructive text-sm font-semibold cursor-pointer flex-shrink-0 ml-4'>
									Delete
								</button>
							</div>
							...
							<div
								...
								<div>
									<div className='flex items-center gap-2'>
										...
										<span
											className={`text-xs font-bold px-2 py-0.5 rounded-full ${item.status === 'Ongoing' ? 'bg-accent-emerald/10 text-accent-emerald' : 'bg-card text-muted-foreground'}`}>
											{item.status}
										</span>
									</div>
									<h4 className='text-lg font-bold text-foreground mt-1'>
						...
						myCourses.map((item) => (
							<div
								...
								<div>
									...
									<span className='ml-2 text-xs text-accent-blue font-bold px-2 py-0.5 rounded-full bg-accent-blue/10'>
										{item.level}
									</span>
									<h4 className='text-lg font-bold text-foreground mt-1'>
										{item.title}
