...
import { Heart, MessageCircle, Share2, Image as ImageIcon, X, ThumbsUp, Send, Loader2, MoreHorizontal, Bookmark } from 'lucide-react'
...
function Avatar({ name, url, size = 10 }: { name: string; url?: string | null; size?: number }) {
  const bg = ['bg-accent-red', 'bg-accent-blue', 'bg-accent-emerald', 'bg-accent-purple']
  const color = bg[name.charCodeAt(0) % bg.length]
  return (
    <div className={`w-${size} h-${size} rounded-full overflow-hidden flex items-center justify-center flex-shrink-0 ${color} text-white font-semibold text-sm`}>
  ...
  return (
    <motion.div
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-0.5">
              <span className="text-xs text-muted-foreground">{post.faculty}</span>
      ...
      <div className="px-5 pb-3">
        <p className="text-foreground text-[15px] leading-relaxed whitespace-pre-line">{post.content}</p>
      </div>

      {/* Image */}
      {post.image_url && (
        <div className="mt-2 overflow-hidden border-t border-border">
          <img src={post.image_url} alt="" className="w-full object-cover max-h-[400px]" />
        </div>
      )}
  ...
  return (
    <section id="community-section" className="bg-secondary/30 dark:bg-secondary/10 min-h-screen py-10">
      <div className="max-w-3xl mx-auto px-4">
        {/* Section heading */}
        <div className="mb-6">
          <p className="text-xs font-semibold text-muted-foreground uppercase tracking-widest mb-2">Collaborate</p>
          <h2 className="font-playfair text-5xl text-foreground mb-2">Research Community</h2>
          ...
          {FILTERS.map(f => (
            <button key={f} onClick={() => setFilter(f)}
              className={`px-4 py-1.5 rounded-full text-xs font-semibold border transition-all cursor-pointer ${
                filter === f ? 'bg-foreground text-background border-foreground' : 'border-border text-muted-foreground hover:border-foreground/30 hover:text-foreground'
              }`}>{f}
            </button>
          ))}
        </div>
        ...
        <div className="bg-card border border-border rounded-2xl p-5 mb-5 shadow-sm">
          <div className="flex items-start gap-3">
            ...
            <button
              ...
              {user ? `What's on your mind, ${name.split(' ')[0]}?` : 'Sign in to share your thoughts...'}
            </button>
          </div>
          ...
          {user && (
            <AnimatePresence>
              <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }}
                className="mt-4 space-y-3" onClick={e => e.stopPropagation()}>
                <textarea value={content} onChange={e => setContent(e.target.value)}
                  placeholder="Share a research update, insight, or question..."
                  rows={3} className="w-full bg-secondary/40 border border-border rounded-xl px-4 py-3 text-foreground text-sm resize-none focus:outline-none focus:ring-2 focus:ring-foreground/15 transition" />
                {imageUrl && (
                  <div className="relative">
                    <img src={imageUrl} alt="" className="rounded-xl max-h-48 object-cover w-full" />
                    <button onClick={() => setImageUrl('')} className="absolute top-2 right-2 bg-black/50 text-white rounded-full p-1 cursor-pointer">
